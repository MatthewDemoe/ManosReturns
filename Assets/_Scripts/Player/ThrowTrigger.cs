...
public class ThrowTrigger : MonoBehaviour
{
    ...
    /// <summary>
    /// Position to aim at. When locked on this thould be the lockon target. When not locked this should be an arbitrary point in front of the player.
    /// </summary>
    [SerializeField]
    ...
    void FixedUpdate()
    {
        ...
        if (Input.GetKeyDown(KeyCode.F) && CoolDebug.hacks)
        {
            maxFootballs = 9999;
            PickUpFootball(9999);
            CoolDebug.GetInstance().LogHack("Many footballs!");
        }
    }
    ...
    void ChargeUp(float c)
    {
        ...
        // if locked onto a target, aim at it
        if(_camMgr.IsLockedOn())
        {
            _targetPosition = _camMgr.GetLockedTarget().position;
        } else // aim forwards
        {
            _targetPosition = transform.position + (cam.transform.forward * _dist);
        }

        int solutions = AppeaseJoss(transform.position, _properspeed, _targetPosition, -Physics.gravity.y, out _lowAngle, out _highAngle);

        // if the target is actually in range, take the low solution to hit it
        if (solutions > 0)
        {
            launchAngle = _lowAngle;
        } else
        {
            // throw at 45 degree angle from the horizontal (farthest throw possible)
            launchAngle = _targetPosition - transform.position;
            launchAngle.y = 0.0f;
            launchAngle.Normalize();

            launchAngle = Quaternion.AngleAxis(-45, transform.right) * launchAngle;

            launchAngle *= _properspeed;
